...
**Testing**: xUnit; NetArchTest.Rules for the architecture suite (MOD-001, MOD-002, COM-001, COM-004,
MON-001, REL-001, TXN-002); Testcontainers for PostgreSQL and RabbitMQ in integration tests;
FluentAssertions; Respawn for per-test database reset

**Target Platform**: Linux container, single host process (modular monolith per STK-002)
...
(research.md R13). No floating-point type may appear in any money path (MON-001). A projected
...
|------|--------|----------------------------|
| SPC-001/002/003 | PASS | `spec.md` passed the keyword scan; every technology decision lives here in `plan.md`. |
| MOD-005 | PASS | Adds the `catalog` module and the `Promotion.Contracts` assembly only. The four-module set is unchanged. |
| MOD-001 | PASS | `Catalog.Infrastructure` references `ECommerce.Promotion.Contracts` and nothing else of Promotion. |
| MOD-002 | PASS | `Catalog.Contracts` and `Promotion.Contracts` hold proto-generated types and port DTOs only — no entity, no `DbContext`, no handler. |
| MOD-003 | PASS | `Shared.Kernel` gets `Money`, `Result`, `IClock`, `PagedResult` — all pass the banking-app test. No `Product` or `Category` type enters shared. |
...
| MON-001 | PASS | `Money` wraps `long` minor units. Architecture test bans `float`, `double`, `decimal` in every money path; columns are `bigint`. |
| MON-002 | N/A | No order exists in this feature. The rule binds the Order module. |
...
| MSG-001 | PASS | The envelope is asserted at the consumer boundary; REL-003's key depends on `message_id`. |
| MSG-003 | PASS | `promotion.discount.changed.v1` is versioned in its name and checked by the CI compatibility gate. |
| TXN-001, TXN-002 | PASS | Read-only path. `TransactionScope` is banned by architecture test regardless. |
| TXN-003, TXN-004 | N/A | No saga in this feature. |
| PRM-001 | PASS | FR-011 — Catalog never calculates a discount. The port is read-only; the adapter has no write method. |
| PRM-002, PRM-004 | N/A | Promotion type logic belongs to the Promotion module. |
| PRM-003 | PASS | The port returns a discriminated result — discount, rejection reason, or unavailable. There is no null and no silent skip. |
| TST-001 | PASS | Every task in `tasks.md` will pair a failing test with its implementation, in that commit order. |
| TST-002 | **PARTIAL** | Domain invariant tests and one test per acceptance criterion apply and are planned. Idempotency tests for money and order writes, and concurrency tests for stock, have no write path in this feature — see Complexity Tracking. |
...
| STK-001 | PASS | No component is added to the stack. The limiter is in the framework and the health checks are in the framework, so no GOV-002 amendment is needed. Making the rate limit exact would need Redis and therefore an amendment — deliberately not taken. |
| GATE-001 | PASS | Architecture suite, migration scanner, and contract tests all run in CI and block the merge. |
| GATE-004 | PASS | Rule identifiers are cited throughout this plan and will be cited in tasks. |
| GOV-007 | N/A | This feature touches no file in the promotion module beyond its `.Contracts` proto. |
...
├── Modules/
│   ├── Catalog/
│   │   ├── ECommerce.Catalog.Contracts/       # events + port DTOs; no entity, no EF type (MOD-002)
...
└── Shared/
    ├── ECommerce.Shared.Kernel/               # Money, Result, IClock, PagedResult (MOD-003)
    └── ECommerce.Shared.Messaging/            # outbox table, relay, inbox consumer base (REL-001..003)

tests/
├── ECommerce.ArchitectureTests/               # MOD-001/002/005, COM-001/004, MON-001, PRM-001,
│                                              # REL-001, TXN-002
├── Catalog/
...
docs/
├── context-map.md                             # module boundaries and event ownership (STK-004)
...
`Contracts`, `Domain`, `Application`, `Infrastructure` — so MOD-001 is checkable by assembly
...
- **MON-001 holds through the wire format.** The proto carries `int64 amount_minor` plus a currency
...
- **PRM-003 holds through the contract shape.** `PricingResult` is a proto `oneof` over `applied`,
...
- **One deviation surfaced by the design and carried forward**: the projection widens FR-014. The
  ...
  compliant with PRM-001 — Catalog only reads from Promotion, never writes — and the copy is
